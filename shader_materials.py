...
class VMDL_OT_load_image(bpy.types.Operator):
    ...
    # OPRAVA: Přistupujeme k materiálu přes aktivní objekt, ne přes kontext
        ...
        def apply_data():
            ...
            for name, path in mat_data.get('textures', {}).items():
                if name in shader_props.textures:
                    try:
                        # Použijeme os.path.normpath pro normalizaci cesty pro daný OS
...
class VMDL_OT_fix_invalid_shader(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        # OPRAVA: Bezpečný přístup k materiálu
        return context.active_object and context.active_object.active_material
...
def setup_principled_node_graph(mat):
    ...
    # Získání objektu, který používá tento materiál (pro přístup k vertex colors)
    # Bereme první objekt, který materiál používá
    ...
    if obj_using_mat and "Color1" in obj_using_mat.data.vertex_colors:
        ...
        if bump_tex_prop and bump_tex_prop.image:
            ...
            links.new(sep_c1.outputs['Blue'], norm_map.inputs['Strength']) # Propojení síly normály
...
class VMDLTextureProperty(bpy.types.PropertyGroup):
    ...
    image: bpy.props.PointerProperty(
        ...
        update=lambda self, context: setup_principled_node_graph(self.id_data) # id_data je materiál
    )
