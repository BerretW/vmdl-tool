...
# --- Globální proměnná pro náhledový materiál ---
preview_material = None

def get_preview_material():
    """Vytvoří nebo vrátí pomocný materiál pro náhled barvy."""
    global preview_material
    if preview_material and preview_material.name in bpy.data.materials:
        return preview_material

    # Hledáme, zda materiál již neexistuje
    mat_name = "VMDL_TINT_PREVIEW_MATERIAL"
    preview_material = bpy.data.materials.get(mat_name)
    if preview_material:
        return preview_material

    # Vytvoříme nový materiál
    preview_material = bpy.data.materials.new(name=mat_name)
    preview_material.use_nodes = True
    # Označíme ho, aby se neukládal do blend souboru, pokud není používán
    preview_material.use_fake_user = True 
    
    nodes = preview_material.node_tree.nodes
    nodes.clear()
    
    # Jednoduchý node setup: Emission node ukazuje barvu
    output_node = nodes.new(type='ShaderNodeOutputMaterial')
    emission_node = nodes.new(type='ShaderNodeEmission')
    emission_node.name = "VMDL_PREVIEW_EMISSION"
    
    preview_material.node_tree.links.new(emission_node.outputs['Emission'], output_node.inputs['Surface'])
    
    return preview_material

def update_tint_preview_color(self, context):
    """
    Přečte barvu z palety na dané pozici a aktualizuje náhled a node graph.
    """
    mat = self.id_data
    if not (mat and mat.use_nodes):
        return

    # 1. Aktualizujeme hodnotu v hlavním node graphu
    preview_node = mat.node_tree.nodes.get("VMDL_TintPreviewValue")
    if preview_node:
        preview_node.outputs[0].default_value = self.tint_preview

    # 2. Přečteme barvu z textury pro UI náhled
    tint_tex_prop = self.textures.get("tintpalettetex")
    if tint_tex_prop and tint_tex_prop.image:
        image = tint_tex_prop.image
        width = image.size[0]
        height = image.size[1]
        
        # Souřadnice pro čtení pixelu
        # X je uprostřed, Y je určeno sliderem
        x = int(width / 2)
        y = int(self.tint_preview * (height - 1))
        
        # Ochrana proti chybě, pokud textura ještě není načtená
        if not image.pixels:
            self.tint_color_preview = (0,0,0,1)
            return

        # Přečteme barvu pixelu
        offset = (y * width + x) * image.channels
        if offset + image.channels <= len(image.pixels):
            color = image.pixels[offset:offset+image.channels]
            # Uložíme barvu do naší property (ujistíme se, že má 4 kanály)
            self.tint_color_preview = color if len(color) == 4 else (color[0], color[1], color[2], 1.0)
        else:
            self.tint_color_preview = (1,0,1,1) # Růžová jako indikátor chyby
    else:
        self.tint_color_preview = (0,0,0,1)

class VMDL_OT_apply_tint_to_object(bpy.types.Operator):
    bl_idname = "vmdl.apply_tint_to_object"; bl_label = "Apply Tint to Object"
    ...
    @classmethod
    def poll(cls, context): obj = context.active_object; return obj and obj.type == 'MESH' and obj.active_material
    def execute(self, context):
        obj = context.active_object; mesh = obj.data
        ...
        for loop_color in color_layer:
            g, b, a = loop_color.color[1], loop_color.color[2], loop_color.color[3]
            loop_color.color = (self.tint_value, g, b, a)
        mesh.update(); self.report({'INFO'}, f"Tint hodnota {self.tint_value:.2f} aplikována."); return {'FINISHED'}

# --- Zbytek souboru (Properties, updaters, Operátory) ---
# ... Všechny ostatní třídy a funkce zůstávají stejné jako v předchozí odpovědi, ...
# ... kromě VMDLShaderProperties, která dostane novou property.

# Přeskočíme na definici VMDLShaderProperties
class VMDLShaderProperties(bpy.types.PropertyGroup):
    shader_name: bpy.props.EnumProperty(
        name="Shader Name", description="Vyberte herní shader", 
        items=lambda s,c: [(n,n,f"Shader: {n}") for n in sorted(SHADER_DEFINITIONS.keys())] or [("NONE", "No Shaders", "")], 
        update=lambda s,c: bpy.app.timers.register(lambda: delayed_shader_update(s,c))
    )
    ...
    tint_preview: bpy.props.FloatProperty(
        name="Tint Preview",
        description="Interaktivně vyberte barvu z palety. Neukládá se, slouží jen pro náhled a aplikaci.",
        min=0.0, max=1.0, default=0.0,
        update=update_tint_preview_color # Používáme novou, chytřejší update funkci
    )

    # <-- NOVÁ PROPERTY pro uložení barvy -->
    tint_color_preview: bpy.props.FloatVectorProperty(
        name="Tint Color Preview",
        subtype='COLOR',
        size=4,
        min=0.0,
        max=1.0,
        description="Aktuálně vybraná barva z palety"
    )

# --- Zbytek souboru může zůstat stejný ---
# Zde je pro jistotu celý soubor:
def setup_principled_node_graph(mat):
    ...
    shader_props = mat.vmdl_shader; shader_name = shader_props.shader_name; x_pos = -1600
    ...
    def find_texture_node(name_part):
        ...
        node = nodes.new('ShaderNodeTexImage'); node.name = f"VMDL_{name_part.capitalize()}Tex"; node.image = prop.image; return node
    ...
    if tint_palette_node:
        ...
        tint_preview_value = nodes.new('ShaderNodeValue'); tint_preview_value.name = "VMDL_TintPreviewValue"; tint_preview_value.outputs[0].default_value = shader_props.tint_preview; tint_preview_value.location = (x_pos + 200, 800)
        ...
        links.new(is_rendering.outputs['Is Camera Ray'], mix_tint_source.inputs['Factor']); links.new(sep_c1.outputs['Red'], mix_tint_source.inputs['A']); links.new(tint_preview_value.outputs['Value'], mix_tint_source.inputs['B'])
        ...
        if base_color_output_socket:
            ...
            links.new(base_color_output_socket, mix_tint.inputs['A']); links.new(tint_palette_node.outputs['Color'], mix_tint.inputs['B']); links.new(mix_tint.outputs['Result'], bsdf.inputs['Base Color'])
    ...
    base_normal_node = find_texture_node("bumptex"); dirt_normal_node = find_texture_node("dirtbumptex") if "Standard_dirt" in shader_name else None
    final_normal_socket = None
    if base_normal_node and dirt_normal_node:
        base_normal_node.image.colorspace_settings.name = 'Non-Color'; base_normal_node.location = (x_pos + 450, -750); dirt_normal_node.image.colorspace_settings.name = 'Non-Color'; dirt_normal_node.location = (x_pos + 450, -1000)
        ...
        links.new(base_normal_node.outputs['Color'], mix_normal.inputs['A']); links.new(dirt_normal_node.outputs['Color'], mix_normal.inputs['B']); links.new(sep_c2.outputs['Red'], mix_normal.inputs['Factor']); final_normal_socket = mix_normal.outputs['Result']
    elif base_normal_node: base_normal_node.image.colorspace_settings.name = 'Non-Color'; base_normal_node.location = (x_pos + 450, -750); final_normal_socket = base_normal_node.outputs['Color']
    ...
    if roughness_node: roughness_node.image.colorspace_settings.name = 'Non-Color'; roughness_node.location = (x_pos + 450, -500); links.new(roughness_node.outputs['Color'], bsdf.inputs['Roughness'])
    else: links.new(sep_c1.outputs['Green'], bsdf.inputs['Roughness'])
class VMDL_OT_clear_texture_slot(bpy.types.Operator):
    bl_idname = "vmdl.clear_texture_slot"; bl_label = "Clear Texture"; bl_description = "Odstraní texturu z tohoto slotu"; bl_options = {'REGISTER', 'UNDO'}; texture_name: bpy.props.StringProperty()
    @classmethod
    def poll(cls, context): return context.material is not None
    def execute(self, context):
        mat = context.material;
        if hasattr(mat, "vmdl_shader") and self.texture_name in mat.vmdl_shader.textures: mat.vmdl_shader.textures[self.texture_name].image = None
        return {'FINISHED'}
class VMDL_OT_load_image(bpy.types.Operator):
    bl_idname = "vmdl.load_image"; bl_label = "Load Image"; bl_description = "Načte obrázek do tohoto slotu"; bl_options = {'REGISTER', 'UNDO'}; filepath: bpy.props.StringProperty(subtype="FILE_PATH"); filter_glob: bpy.props.StringProperty(default="*.png;*.jpg;*.jpeg;*.tga;*.bmp;*.dds", options={'HIDDEN'}); texture_name: bpy.props.StringProperty()
    def execute(self, context):
        mat = context.active_object.active_material; tex_prop = mat.vmdl_shader.textures.get(self.texture_name)
        try: tex_prop.image = bpy.data.images.load(self.filepath, check_existing=True); self.report({'INFO'}, "Obrázek načten.")
...
class VMDLTextureProperty(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(name="Texture Name"); image: bpy.props.PointerProperty(name="Image", type=bpy.types.Image, update=lambda self, context: setup_principled_node_graph(self.id_data))
class VMDLParameterProperty(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(); type: bpy.props.StringProperty(); float_value: bpy.props.FloatProperty(); vector_value: bpy.props.FloatVectorProperty(size=4, subtype='COLOR'); bool_value: bpy.props.BoolProperty()
def get_shader_enum_items(self, context):
    items = [(n, n, f"Shader: {n}") for n in sorted(SHADER_DEFINITIONS.keys())]; return items if items else [("NONE", "No Shaders", "")]
def delayed_shader_update(self, context):
    ...
    if self.shader_name in SHADER_DEFINITIONS:
        s_def = SHADER_DEFINITIONS[self.shader_name]
        for p_def in s_def.get("parameters", []):
            p = self.parameters.add(); p.name=p_def["name"]; p.type=p_def["type"]
            if p.type=="float": p.float_value=p_def["default"]
            elif p.type=="vector4": p.vector_value=p_def["default"]
            elif p.type=="bool": p.bool_value=p_def["default"]
        for t_def in s_def.get("textures", []): self.textures.add().name = t_def["name"]
...
class VMDL_OT_save_material_preset(bpy.types.Operator, ExportHelper):
    bl_idname = "vmdl.save_material_preset"; bl_label = "Save Preset"; filename_ext = ".mat.json"; filter_glob: bpy.props.StringProperty(default="*.mat.json", options={'HIDDEN'})
    @classmethod
    ...
    def execute(self, context):
        ...
        with open(self.filepath, 'w') as f: json.dump(data, f, indent=4); self.report({'INFO'}, "Preset uložen."); return {'FINISHED'}
class VMDL_OT_load_material_preset(bpy.types.Operator, ImportHelper):
    bl_idname = "vmdl.load_material_preset"; bl_label = "Load Preset"; filename_ext = ".mat.json"; filter_glob: bpy.props.StringProperty(default="*.mat.json", options={'HIDDEN'})
    @classmethod
    ...
    def execute(self, context):
        ...
        except: self.report({'ERROR'}, "Chyba čtení"); return {'CANCELLED'}
        ...
        def apply_data():
            ...
            for n,p in data.get('textures', {}).items():
                if n in props.textures:
                    ...
                    except: self.report({'WARNING'}, "Nelze načíst texturu")
            self.report({'INFO'}, "Preset načten.")
...
class VMDL_OT_fix_invalid_shader(bpy.types.Operator):
    ...
    def poll(cls, context): return context.active_object and context.active_object.active_material
    def execute(self, context):
        if SHADER_DEFINITIONS: context.active_object.active_material.vmdl_shader.shader_name = sorted(SHADER_DEFINITIONS.keys())[0]; self.report({'INFO'}, "Shader opraven.")
        return {'FINISHED'}
class VMDL_MT_create_material_menu(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if not keys: self.layout.label(text="Žádné shadery!", icon='ERROR'); return
        for name in keys: self.layout.operator("vmdl.create_shader_material", text=name).shader_name_prop = name
class VMDL_OT_create_shader_material(bpy.types.Operator):
    bl_idname = "vmdl.create_shader_material"; bl_label = "Create VMDL Shader Material"; bl_options = {'REGISTER', 'UNDO'}; shader_name_prop: bpy.props.StringProperty()
    @classmethod
    ...
    def execute(self, context):
        ...
        obj.data.materials.append(mat); obj.active_material = mat; mat.vmdl_shader.shader_name = self.shader_name_prop
        self.report({'INFO'}, f"Materiál '{mat.name}' vytvořen."); return {'FINISHED'}